...
export default function DemoPage() {
  ...
  const [demoOrgId, setDemoOrgId] = useState<string>('demo-org');
  const demoOrgs = useMemo(() => [{ org_id: 'demo-org', name: 'Demo Exporters' }], []);
  ...
  const generatePlan = (nextIntent?: string) => {
    ...
    const text = (nextIntent ?? intent).trim();
    setMessages((m) => [...m, { role: 'user', text }, { role: 'assistant', text: 'Working on a Trade Plan…' }]);
    ...
    >
      <div className="p-6 max-w-6xl mx-auto">
        <WorkspaceGrid
          left={
            <ChatPane
              title="Trade Assistant (Demo)"
              subtitle="PT→ES • mock flow"
              messages={messages}
            ...
            <>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-2 text-xs text-muted">
                      Anchoring: <span className="text-ink font-medium">ON</span> • STF:{' '}
                      <span className="text-ink font-medium">ON</span>
                    </div>
                  </div>
                  <Button
                    variant="primary"
                    onClick={runNextCta}
                    disabled={nextCta.action === 'none'}
                  >
              ...
              >
                {plan.status === 'ready' ? (
                  <div className="text-sm space-y-2">
                    <Field
                      label="Items"
                      value={`${plan.items?.[0]?.qty} ${plan.items?.[0]?.unit} — ${plan.items?.[0]?.name} (HS ${plan.items?.[0]?.hs_code})`}
                    />
              ...
              <TradeCard
                ...
                primary={{
                  ...
                  disabled:
                    !(compliance.status === 'passed' || compliance.status === 'warnings') ||
                    finance.status === 'requesting' ||
                    finance.status === 'accepted'
                }}
                  ...
                  <div className="space-y-2">
                    ...
                    {finance.reservation ? (
                      <div className="text-xs text-muted">
                        Reservation: {finance.reservation.offer_id} • expires{' '}
                        {new Date(finance.reservation.expires_at).toLocaleTimeString()}
                      </div>
                    ) : null}
                  </div>
                ) : (
              ...
              <TradeCard
                ...
                primary={{
                  label: !payments.bank_linked
                    ? 'Connect bank'
                    : payments.status === 'idle'
                      ? 'Compute routes'
                      : payments.status === 'routes_ready'
                        ? 'Execute payment'
                  ...
                  disabled:
                    finance.status !== 'accepted' ||
                    payments.status === 'routing' ||
                    payments.status === 'executing' ||
                    payments.status === 'executed'
                }}
                secondary={
              ...
              >
                {!payments.bank_linked ? (
                  <p className="text-sm text-muted">
                    Connect a bank account (AIS/PIS) to compute routes and execute payments.
                  </p>
                ) : payments.routes ? (
                  <div className="space-y-2">
                    ...
                    {payments.routes.map((r) => (
                      <div
                        key={r.route_id}
                        className="flex items-center justify-between rounded-xl border border-border/10 bg-surface2/40 p-3 text-sm"
                      >
                        <div>
                    ...
                    {payments.payment ? (
                      <div className="text-xs text-muted break-all">
                        Payment: {payments.payment.payment_id} • {payments.payment.status}{' '}
                        {payments.payment.iso_status ? `(${payments.payment.iso_status})` : ''}
                      </div>
                    ) : null}
                  </div>
                ) : (
              ...
              <TradeCard
                ...
                status={
                  proofs.status === 'ready'
                    ? {
                        label: proofs.anchor?.status === 'anchored' ? 'Anchored' : 'Ready',
                        tone: proofs.anchor?.status === 'anchored' ? 'success' : 'neutral'
                      }
              ...
              >
                {proofs.status === 'ready' ? (
                  ...
                  <p className="text-sm text-muted">
                    Generate an exportable, verifiable proof bundle and optional chain receipt.
                  </p>
                )}
              </TradeCard>

              <details className="rounded-2xl border border-border/10 bg-surface1/60 px-4 py-3">
                <summary className="cursor-pointer text-xs text-muted select-none">Recent events</summary>
...
function Message({ role, text }: { role: 'user' | 'assistant'; text: string }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm border ${isUser ? 'bg-ink text-paper border-border/10' : 'bg-surface1 border-border/10'}`}>
        {text}
      </div>
    </div>
  );
...
function NavItem({ icon, label, active }: { icon: React.ReactNode; label: string; active?: boolean }) {
  return (
    <div className={`flex items-center gap-2 rounded-xl px-3 py-2 ${active ? 'bg-border/5 font-medium' : 'text-ink/90 hover:bg-border/5'} cursor-default`}>
...
function DrawerItem({ label, tone }: { label: string; tone: StatusTone }) {
  return (
    <div className="rounded-2xl border border-border/10 bg-surface1 p-4 flex items-center justify-between">

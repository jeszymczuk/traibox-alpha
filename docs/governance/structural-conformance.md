...
| Protected actions | exact shared-contract/manifest identifiers; immutable critical gates; existing action-specific route, contract, authorization, consumption, guarded-service, denial, DB integration, replay, and concurrency evidence for `send_payment` and `accept_funding_offer`; live-provider import boundaries | runtime repair, approval binding, Finance/Payments policy, provider invocation, or marking either critical finding resolved |

...
export async function getOrBuildBundle(
  ...
  if (existing) {
    ...
    const artifacts = await listProofArtifacts(pool, { orgId: input.orgId, userId: input.userId, tradeId: input.tradeId });
    return {
      bundle_url: existing.bundle_url,
      manifest_sha256: existing.manifest_sha256,
      root: existing.root,
      created_at: toIso(existing.created_at),
      artifacts,
  ...
  const proofArtifacts = artifacts.map((artifact) => ({
    path: artifact.path,
    mime: artifact.mime,
    bytes: artifact.data.byteLength,
    sha256: artifact.hashing === 'jcs-json' ? sha256Hex(canonicalJsonBytes(JSON.parse(artifact.data.toString('utf8')))) : sha256Hex(artifact.data)
  }));
  return { bundle_url: bundleUrl, manifest_sha256: out.manifestSha256, root: out.root, created_at: createdAt, artifacts: proofArtifacts, artifact_count: proofArtifacts.length, anchor, trace_id: input.traceId };
}
...
export async function verifyStoredBundle(
  ...
  const bundle = await withTx(pool, async (client) => {
    ...
    if (input.bundleUrl) {
      const res = await client.query(
        'SELECT trade_id, bundle_url, root, manifest_sha256 FROM proof_bundles WHERE org_id=$1 AND bundle_url=$2 ORDER BY created_at DESC LIMIT 1',
        [input.orgId, input.bundleUrl]
      );
      return res.rows[0] ?? null;
    }
    const res = await client.query(
      'SELECT trade_id, bundle_url, root, manifest_sha256 FROM proof_bundles WHERE org_id=$1 AND trade_id=$2 ORDER BY created_at DESC LIMIT 1',
      [input.orgId, input.tradeId]
    );
    return res.rows[0] ?? null;
  });
  if (!bundle) {
    const error = new Error('Proof bundle not found');
    (error as any).statusCode = 404;
    throw error;
  }

  const bytes = await storage.getObjectByUrl(bundle.bundle_url);
  const verification = await input.verifier(bytes);
  const rootMatches = verification.root === bundle.root;
  const reasons = [...verification.reasons, ...(rootMatches ? [] : ['Stored root mismatch'])];
  const out: LedgerVerifyStoredResponse = {
    valid: verification.valid && rootMatches,
    reasons,
    root: verification.root,
    anchored: false,
    bundle_sha256: verification.bundleSha256,
  ...
  await writeLedgerMemory(pool, {
    orgId: input.orgId,
    userId: input.userId,
    traceId: input.traceId,
    tradeId: bundle.trade_id ?? null,
    kind: 'ledger.bundle.verified',
    signal: out.valid ? 'proof.ledger_verified' : 'proof.ledger_verification_failed',
    payload: {
      bundle_url: out.bundle_url,
      valid: out.valid,
      root: out.root,
      expected_root: out.expected_root,
  ...
  await insertEvent(pool, {
    ...
    ev: {
      event_id: crypto.randomUUID(),
      type: 'ledger.bundle.verified',
      ts: out.verified_at,
      org_id: input.orgId,
      trade_id: bundle.trade_id ?? undefined,
      trace_id: input.traceId,
      actor: `user:${input.userId}`,
      data: {
        trade_id: bundle.trade_id ?? null,
        bundle_url: bundle.bundle_url,
        valid: out.valid,
        root: out.root,
        expected_root: out.expected_root,
...
export async function exportLedger(
  ...
  await insertEvent(pool, {
    ...
    ev: {
      event_id: crypto.randomUUID(),
      type: 'ledger.export.ready',
      ts: new Date().toISOString(),
      org_id: input.orgId,
      trace_id: input.traceId,
      actor: `user:${input.userId}`,
      data: { trade_ids: input.tradeIds, url, hash, trace_id: input.traceId }
    }
  });
  await writeLedgerMemory(pool, {
    orgId: input.orgId,
    userId: input.userId,
    traceId: input.traceId,
    tradeId: input.tradeIds[0] ?? null,
    kind: 'ledger.export.ready',
    signal: 'proof.audit_export_ready',
...
async function listProofArtifacts(pool: pg.Pool, input: { orgId: string; userId: string; tradeId: string }): Promise<LedgerProofArtifact[]> {
  return withTx(pool, async (client) => {
    await setAppContext(client, { userId: input.userId, orgId: input.orgId });
    const res = await client.query(
      `SELECT path, mime, bytes, sha256, created_at
       FROM proof_artifacts
       WHERE org_id=$1 AND trade_id=$2
...
async function writeLedgerMemory(
  pool: pg.Pool,
  input: {
    orgId: string;
    userId: string;

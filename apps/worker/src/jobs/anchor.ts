...
async function submitAnchor(input: { pool: pg.Pool; orgId: string; batchId: string; root: string; network: string; contract: ethers.Contract }) {
  ...
  const anchorContract = input.contract as ethers.Contract & {
    anchor(root: string, memo: string): Promise<{ hash: string }>;
  };
  const tx = await anchorContract.anchor(rootBytes32, memo);

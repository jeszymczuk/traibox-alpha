...
- Logout occurs only after an explicit user click submits `POST`; rendering or navigating to `/logout` performs no mutation. The POST requires exact-origin and CSRF validation, attempts Supabase logout for user sessions, revokes the local row, and clears both possible cookie names. Provider failure cannot preserve the local browser session and is reported only as a non-sensitive confirmation flag.

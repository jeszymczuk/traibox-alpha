...
import { Moon, Sparkles, Sun, LayoutGrid, HandCoins, WalletCards, ShieldCheck, Receipt, Landmark, Settings, Users } from 'lucide-react';
      ...
      <aside className="border-r border-border/10 bg-surface1/60 backdrop-blur p-4">
        ...
        <div className="mt-4 space-y-1 text-sm">
          ...
          <div className="pt-2">
            ...
            <NavStatic icon={<Sparkles className="h-4 w-4" />} label="Trade Assistant" />
            <NavStatic icon={<ShieldCheck className="h-4 w-4" />} label="Compliance" />
            <NavStatic icon={<HandCoins className="h-4 w-4" />} label="Finance" />
            <NavStatic icon={<WalletCards className="h-4 w-4" />} label="Payments" />
            <NavStatic icon={<Receipt className="h-4 w-4" />} label="Proofs" />
            <NavStatic icon={<Landmark className="h-4 w-4" />} label="Network" />
            <NavStatic icon={<Settings className="h-4 w-4" />} label="Settings" />
          </div>
...
function NavStatic({ icon, label, active }: { icon: React.ReactNode; label: string; active?: boolean }) {
  return (
    <div
      className={cn(
        'flex items-center gap-2 rounded-xl px-3 py-2 cursor-default select-none',
        active ? 'bg-border/5 font-medium' : 'text-ink/90 hover:bg-border/5'
      )}
    >
